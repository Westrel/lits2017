...
for file_index, file in enumerate(os.listdir(para.test_ct_path)):
    ...
    # 将预测的结果保存为nii
    # print(liver_seg.shape, seg_array.shape, pred_seg.shape, start_slice, end_slice)
    liver_seg = liver_seg*0.67 + seg_array*0.33 ####
    ...
    print("Val: ", file_index, "this case use {:.3f} s.".format(speed))
    print('------------------------------')
...
writer = pd.ExcelWriter(para.result_path)
liver_data.to_excel(writer, 'liver')
liver_statistics.to_excel(writer, 'liver_statistics')
writer.save()

# 打印dice global
print('Val: dice global is ', dice_intersection/dice_union)